...
python -m torch.distributed.launch --nproc_per_node=1 --master_port=$((RANDOM%10000+20000)) --use_env main.py --global_model 'meta-llama/Llama-3.2-1B' --data_path  "./data_alpaca" --output_dir './Llama-3.2-1B-alpaca-homo-75-1-100-florist/' --num_communication_rounds 75 --local_num_epochs 1 --method florist --num_clients 100

python -m torch.distributed.launch --nproc_per_node=1 --master_port=$((RANDOM%10000+20000)) --use_env main.py --global_model 'meta-llama/Llama-3.2-1B' --data_path  "./data_alpaca" --output_dir './Llama-3.2-1B-alapca-homo-75-1-100-flora/' --num_communication_rounds 75 --local_num_epochs 1 --method flora --num_clients 100
